// src/lib/sdk.ts
//
// Munshot Dashboard SDK client adapter — the canonical integration prescribed by
// the Munshot dashboard auth standard. Typed against the shipped bundle
...
//
// Rules this file encodes (do not deviate):
//   • exactly one client, created at module load (this file);
//   • autoReady left at its default (true) — the SDK sends dashboard:ready
//     itself from inside its host:init handler. NEVER call ready() manually;
//   • context is read with getContext() and re-synced via onMessage();
//   • the SDK <script> is loaded as a classic blocking tag in index.html, so
//     window.MunshotDashboardSDK is defined before this module is imported.
...
// build standalone outside the Munshot host, or in the node test runner where
// there is no window). Return types match the real client so app code behaves
// identically.
...
function initSdk(): DashboardClientSdk {
  // No window (SSR / node test runner) → behave exactly as if the host is absent.
  if (typeof window === 'undefined') return createNoopSdk()
